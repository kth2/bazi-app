...
1. **「第三者之扰」要不要也默认关闭？** 它不在点名的四类里，所以目前是
   `normal`、照常显示；但它是对配偶的断言，敏感度上与「离婚风险」同档。
   没有擅自加进默认关闭名单——那与「破财风险不要默认关闭」的口径相反。

2. **组合判据规则**（见 13.3）：要不要给 `_EventRule` 加「多目标同时成立」
   的表达能力？这是打开留学/买房/名气/手术/官司五个类型的唯一途径。
